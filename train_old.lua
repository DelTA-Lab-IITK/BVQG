...
-- Data input settings
cmd:option('-input_img_train_h5','../../ref_coco_joint_mul_add_mul_place_tag_moe3_tv_gate/data/img_train_fc7.h5','path to the h5file containing the image feature')
cmd:option('-input_img_test_h5','../../ref_coco_joint_mul_add_mul_place_tag_moe3_tv_gate/data/img_test_fc7.h5','path to the h5file containing the image feature')
cmd:option('-input_ques_h5','../../ref_coco_joint_mul_add_mul_place_tag_moe3_tv_gate/data/coco_data_prepro_pos_tv.h5','path to the h5file containing the preprocessed dataset')
cmd:option('-input_json','../../ref_coco_joint_mul_add_mul_place_tag_moe3_tv_gate/data/coco_data_prepro_pos_tv.json','path to the json file containing additional info and vocab')
...
cmd:option('-mc_sample', 2, 'which gpu to use. -1 = use CPU')
...
-- Encoding Part
        ...
        -- jointtion feature embedding
        protos.ale = nn.Sequential()
                :add(nn.View(-1,lmOpt.vocab_size+1))
                :add(nn.Linear(lmOpt.vocab_size+1, 1))
                :add(nn.SoftPlus())


         -- jointtion feature embedding
        protos.linearLayer = nn.Sequential()
                :add(nn.View(-1,lmOpt.vocab_size+1))
                :add(nn.Linear(lmOpt.vocab_size+1, lmOpt.vocab_size+1))

        protos.myview=nn.Sequential()
                :add(nn.View(-1))
        ---------------------------------------------------------------------------------------------
print('lmOpt.vocab_size',lmOpt.vocab_size)
   ---------------------------------------------------------------------------
		--moe3 part
        -- Place feature Image feature embedding
...
protos.criterion_bayesian = bayesian_crossentropy.criterion(opt.mc_sample)
...
local aparams, grad_aparams = protos.ale:getParameters()
local linear_param,grad_linear=protos.linearLayer:getParameters()
...
aparams:uniform(-0.1, 0.1)
linear_param:uniform(-0.1,0.1)
...
if string.len(opt.start_from) > 0 then
  ...
  linear_param:copy(loaded_checkpoint.linear_param)
...
print('total number of parameters in ale net: ', aparams:nElement())
...
-------------------------------------------------------------------------------------------------------
          ...
          for y = 1, 100 do
              ...
              local logprobs1 = protos.lm:forward({moe_feat, decode_question}) -- data.questions=data.labels, img_feat=expanded_feats
              -- print('logprobs',logprobs:size())
              -- forward the language model
              local logprobs=protos.linearLayer:forward(logprobs1)
              local logprobs_variance = protos.ale:forward(logprobs1) -- data.questions=data.labels, img_feat=expanded_feats
              -- print('decode_question',decode_question:size())

              softmax:clearState()
              softmax:zeroGradParameters()
              local prob_cls = softmax:forward(logprobs)
              local log_prob_cls = logsoftmax:forward(logprobs:clone())
              local one_tensor_cls = torch.CudaTensor(opt.batch_size, lmOpt.vocab_size):fill(1)
              local grads_entropy_cls = -torch.add(log_prob_cls, one_tensor_cls)/opt.batch_size

              epistemtic_cls:add(-torch.cmul(prob_cls, log_prob_cls):sum(2))
              aleatoric_cls:add(logprobs_variance:clone())

              --- attention from discriminator using predictive uncertainty (certainty)
              local total_grads_entropy_cls = softmax:backward(logprobs, grads_entropy_cls)
              local d_logprobs=protos.linearLayerbackward(logprobs1,total_grads_entropy_cls)
              local d_logprobs_variance = protos.ale:backward(logprobs1,logprobs_variance)

              dgradOutputs_uncertainty = d_logprobs + d_logprobs_variance

              d_logprobs1=protos.lm:backward({moe_feat, decode_question},dgradOutputs_uncertainty)
        ...
        --   -- forward the language model criterion
        --   local loss = protos.crit:forward(logprobs, decode_question)
        --   --  print('logprobs_variance',logprobs_variance:size())

        --   -- decode_question=protos.myview:forward(decode_question)
        --   -- print('decode_question',decode_question:size())
        --   std=nn.Sqrt():cuda():forward(logprobs_variance):cuda()
        --   -- print('std',std)
        --     sam=(nn.Sampler():cuda()):forward(std)
        --     -- print('sam',sam[1])
        --     -- print('logprobs',logprobs:size())
        --     add=(nn.CAddTable():cuda()):forward({logprobs,sam})
        --     -- print('add',add:size())

        --   local distorted_loss = protos.crit:forward(add, decode_question)
        --     -- print('distorted_loss',distorted_loss)

        --     local expVar=nn.Exp():cuda():forward(logprobs_variance)
        --     local subs= nn.CSubTable():cuda():forward(expVar, torch.CudaTensor(opt.batch_size):fill(1))

        -- local variance_depressor = nn.Mean():cuda():forward(subs)
    ------ undistorted loss ---------------------------
    -- local undistorted_loss = nn.CrossEntropyCriterion()({pred_logit, true_label})
    -- ----- montecarlo simulations ----------------------
    -- local monte_carlo_results = {}
    -- for i = 1, opt.mc_sample do
    --     ----- distorted loss = Cross_entropy(pred_logit + sampled noise, true label)
    --         local distorted_loss = protos.crit:forward(add, decode_question)        --- diff = Elu(undistorted loss - distorted loss) -----------

    --         print('loss',loss)
    --         print('distorted_loss',distorted_loss)
    --         local diff=nn.CSubTable():cuda():forward({loss, loss})
    --         local elll=nn.ELU(1):cuda():forward(diff)

    --     table.insert(monte_carlo_results, elll)
    -- end
    ...
    -- local output = nn.CAddTable()({variance_loss, undistorted_loss, variance_depressor})
          ...
          ---aleatoric loss
           -- variance_depressor,undistorted_loss = protos.criterion_bayesian:forward({logprobs, decode_question, logprobs_variance, torch.CudaTensor(opt.batch_size):fill(1), torch.CudaTensor(1):fill(opt.mc_sample)})[1]
           -- print('variance_depressor',variance_depressor.size())
           -- print('monte_carlo_results',monte_carlo_results)
           -- print('undistorted_loss',undistorted_loss.size())
           -- print('err_aleatoric',output.size())
        -------------------------------------------------------------------------------------------------------------------
-----------------------------------------------------------------------------
-- Backward pass
-----------------------------------------------------------------------------
        ...
        local d_lm_feats, ddummy = unpack(protos.lm:backward({moe_feat, decode_question}, d_logprobs1))
----------------------------------------------------------------------------------------------
