...
function mul_constant_tenser:__init(constant_scalar,opt.batch_size)
  parent.__init(self)
...
--   print('self.constant_scalar',self.constant_scalar)
-- print('self.output',self.output)
    for i=1,self.batch_size do
          --print('self.constant_scalar',self.constant_scalar[i][1])

          self.output[i]:mul(self.constant_scalar[i][1])
          --print('self.output',self.output[i])
    end
...
function mul_constant_tenser:updateGradInput(input, gradOutput)
    ...
    for i=1,self.batch_size do
    self.gradInput[i]:mul(self.constant_scalar[i][1])
    end
    print('self.output',self.gradInput)
